...
bool RunMP(const char *clientDllPath)
{
	...
	mono.mono_add_internal_call ("MSCO.Client::ShowMessageBox", ShowMessageBox);
	...
	MonoClass* monoClass = mono.mono_class_from_name(image, "MSCO", "Client");
	if (!monoClass)
	{
		return false;
	}
...
{
	// If MSCO is not build in debug configuration require explicit
...
void Injector_RunMP()
{
	...
	{
		MessageBox(NULL, "Failed to run multiplayer mod!", "MSCO", MB_ICONERROR);
		ExitProcess(0);
	}
}
...
{
	MessageBox(NULL, message, "MSCO", MB_ICONERROR);
	ExitProcess(0);
}
...
static bool SetupUnityLog()
{
	...
	// Fallback, try to create unityLog in MSCO installation directory.

	char unityLogPath[MAX_PATH] = { 0 };
	GetModulePath(GetModuleHandle("MSCOInjector.dll"), unityLogPath);
...
BOOL WINAPI DllMain(HMODULE hModule, unsigned Reason, void *Reserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		GetModulePath(GetModuleHandle("MSCOInjector.dll"), ClientDllPath);
		strcat(ClientDllPath, "\\MSCOClient.dll");
