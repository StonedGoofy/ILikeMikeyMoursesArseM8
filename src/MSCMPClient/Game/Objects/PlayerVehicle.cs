...
using UnityEngine;
using MSCO.Game.Components;
using HutongGames.PlayMaker;

namespace MSCO.Game.Objects {
	class PlayerVehicle : ISyncedObject {
		...
		PlayMakerFSM doorLeft = null;
		PlayMakerFSM doorRight = null;
		...
		public void FindFSMs() {
			...
			foreach (var fsm in fsms) {
				...
				// Door left
				if (fsm.gameObject.name == "DoorLeft" && fsm.FsmName == "Use")
				{
					doorLeft = fsm;
				}

				// Door right
				if (fsm.gameObject.name == "DoorRight" && fsm.FsmName == "Use")
				{
					doorRight = fsm;
				}
			}
