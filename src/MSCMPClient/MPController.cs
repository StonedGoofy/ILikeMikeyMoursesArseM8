...
namespace MSCO {
	...
	class MPController : MonoBehaviour {
		...
		void Start() {
			...
			modLogo = Client.LoadAsset<Texture2D>("Assets/Textures/MSCOLogo.png");
		...
		void OnGUI() {
			...
			GUI.Label(new Rect(2, Screen.height - 18, 500, 20), "MSCO " + Client.GetMODDisplayVersion());
		...
		/// </summary>
		const float FRIENDLIST_UPDATE_INTERVAL = 1f;

		/// <summary>
		/// Time left to next friend update.
		/// </summary>
		float timeToUpdateFriendList = 0.1f;
		...
		/// </summary>
		const float INVITE_COOLDOWN = 0f;
		...
		private void UpdateInvitePanel() {
			if (!IsInvitePanelVisible()) {
				GUI.color = Color.white;
				GUI.Label(new Rect(0, Screen.height - 100, 200.0f, 20.0f), "Click ESC to Invite Friends");
				return;
			}
			...
			GUI.color = new Color(0.0f, 0.1f, 0.9f, 0.8f);
