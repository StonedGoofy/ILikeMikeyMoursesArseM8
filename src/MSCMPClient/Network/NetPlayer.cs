...
namespace MSCO.Network {
	...
	class NetPlayer : IDisposable {
		...
		PlayerAnimManager Anim = new PlayerAnimManager();
		...
		private void SitInCurrentVehicle() {
			if (currentVehicle != null) {
				...
				//Anim.PlayAnimation(PlayerAnimManager.AnimationId.CrouchingLow, true, true);
				if (IsSpawned) {
		...
		public virtual void EnterVehicle(Game.Components.ObjectSyncComponent vehicle, bool passenger) {
			///Message box below (comment) is now replaced with an If statement to output error to Chat.
			///MAY CAUSE LOW FPS ISSUES
			...
			if (currentVehicle != null)
			{
				MessagesList.AddMessage($"Can not leave Driving mode. Stop moving and unfasten seatbelt!", MessageSeverity.Info);
			}
			else if (state != State.OnFoot)
			{
				MessagesList.AddMessage($"Entered vehicle but player is not on foot.", MessageSeverity.Info);
			}
		...
		public virtual void LeaveVehicle() {
			if (currentVehicle != null && state == State.OnFoot)
			{
				MessagesList.AddMessage($"Player is leaving vehicle but he is not in vehicle.", MessageSeverity.Info);
			}
