...
namespace MSCO.Network {
	...
	class PlayerAnimManager {
		...
		public byte GetActiveHandState(GameObject gameObject) {
			/*GameObject HandHandleObject = gameObject.transform.FindChild("Pivot/Camera/HandCamera").gameObject;
		...
		private class JumpState : AnimState {
			...
			public override void Activate() { PlayAnimation(AnimationId.Jumping, true, true); }
