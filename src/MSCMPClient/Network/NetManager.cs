...
namespace MSCO.Network {
	class NetManager {
		...
		/// </summary>
		const float HEARTBEAT_INTERVAL = 0.5f;
		...
		private void HandleDisconnect(bool timeout) {
			...
			if (IsHost) {
				...
				MessagesList.AddMessage($"Player {players[1].GetName()} disconnected. ({reason})", MessageSeverity.Error);
			}
			CleanupPlayer();
		...
		private void ProcessMessages() {
			...
			while (Steamworks.SteamNetworking.IsP2PPacketAvailable(out size)) {
				...
				if (protocolId != PROTOCOL_ID) {
					Logger.Error("The received message was not sent by MSCO network layer.");
					continue;
				}
		...
		private void HandleHandshake(Steamworks.CSteamID senderSteamId, Messages.HandshakeMessage msg) {
			if (IsHost) {
				...
				MessagesList.AddMessage($"Player {players[1].GetName()} joined.", MessageSeverity.Player);
			}
			else {
				if (players[1] == null) {
