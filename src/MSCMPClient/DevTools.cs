	...
	static class DevTools {
		...
		public static void OnInit() {
			...
			UI.Console.RegisterCommand("help", (string[] args) =>
			{
				...
				Client.ConsoleMessage($"   getcars");
				Client.ConsoleMessage($"   removesatsuma");
			});
			...
			//Teleports Gifu, Hayosiko and Ruscko Home.
			UI.Console.RegisterCommand("getcars", (string[] args) => {
				GameObject gifu = GameObject.Find("GIFU(750/450psi)"); 
				gifu.transform.rotation = new Quaternion(0.0076284916f, 0.0000877945f, 0.0002504693f, 0.9999708533f);
				gifu.transform.position = new Vector3(-19.9816398621f, -0.3939682746f, 19.8360805511f);

				GameObject hayosiko = GameObject.Find("HAYOSIKO(1500kg, 250)");
				hayosiko.transform.rotation = new Quaternion(0.0015769413f, -0.0000921047f, 0.0064880662f, 0.9999777079f);
				hayosiko.transform.position = new Vector3(-19.8052330017f, -0.2911319041f, 10.6132068634f);

				GameObject ruscko = GameObject.Find("RCO_RUSCKO12(270)");
				ruscko.transform.rotation = new Quaternion(0.0018048146f, 0.0005752785f, -0.0003549336f, 0.9999981523f);
				ruscko.transform.position = new Vector3(-16.5002117157f, -0.1301583552f, 2.8128595352f);

				GameObject kekmet = GameObject.Find("KEKMET(350-400psi)");
				kekmet.transform.rotation = new Quaternion(0.0066924575f, -0.4891268015f, 0.0043625906f, 0.8721761703f);
				kekmet.transform.position = new Vector3(-6.2257871628f, -0.1162529439f, 18.4869747162f);

				Client.ConsoleMessage($"Gifu, Hayosiko and Ruscko transported to Home!");
			});
			//Clears Satsuma
			UI.Console.RegisterCommand("removesatsuma", (string[] args) => {
				GameObject satsuma = GameObject.Find("SATSUMA(557kg, 248)");
				satsuma.transform.position = new Vector3(2889.0f, -1.0f, 2250.0f);

				Client.ConsoleMessage($"Satsuma Removed.");
			});
